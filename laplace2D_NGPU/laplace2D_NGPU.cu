	  ...
	  float* phiT_old, float* phi0_new, float* C)
{
    ...
    if (skip == 0) {
      phi0_new[site] = 0.25*w*(b+l+r+t)+(1-w)*phi0_old[site];
...
int op(int NGPU,int* Dev, int Nx, int Ny, int Lx, int Ly, int NGx, int NGy, int tx, int ty, int CPU , std::ofstream& myfile)
{
...
//  Copy result from device memory to host memory
	...
	myfile<<Nx<<"x"<<Ny<<","<<tx<<"x"<<ty<<","<<gputime<<","<<gputime_tot<<std::endl;
    fflush(stdout);
...
//  Compute the CPU solution.
	...
	while ( (error > eps) && (iter < MAX) ) {
	    ...
	    for (int x=0; x<Nx; x++) { 
		    ...
		    r = h_old[xp1]; 
		    t = h_old[yp1]; 
		    h_new[site] = 0.25*w*(b+l+r+t)+(1-w)*h_old[site];
...
int main(void)
{
 ...
 //  Get the input parameters.
	...
	{
		printf("loop: %d\n",i);
		op(NGPU, Dev, Nx, Ny, Lx, Ly, NGx, NGy, ts[i], ts[i], CPU, myfile);
	}
