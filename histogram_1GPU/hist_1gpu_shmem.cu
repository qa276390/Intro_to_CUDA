...
//    RandomNormal(data_h, N);       // Gaussian deviate with sigma = 1
   ...
   // start the timer
    ...
    if(CPU==0) {
	  printf("Quit without CPU Computation\n");
    ...
    printf("To compute the CPU reference solution...\n"); 
...
void RandomExp(float* data, long n)   // RNG with normal distribution, mu=0, sigma=1
{
//	float max = 0.0;
    for(long i = 0; i < n; i++) {
      double y = (double) rand() / (float)RAND_MAX;
